...
function getRangeLegend(colorArr, valueRangeArr) {
    var map_scaleHtml = '<img src="./Chinese_Stock_Treemap_files/right.png" alt="" id="legend-close" class="legend-switch"><img src="./Chinese_Stock_Treemap_files/left.png" alt="" id="legend-open" class="legend-switch">';
...
function getparam() {
    ...
    if (param != undefined) {
        if (param.fullscreen == 0) {
            document.getElementById("enlage").click();
        } else if (param.fullscreen == 1) {
            document.getElementById("narrow").click();
        }
        if (param.datatype < 16) {
            document.getElementById('select-change').selectedIndex = param.datatype;
            document.getElementById('select-change').click;
        }
        for (var i = 0; i < 4; i++) {
            if (param.stocktype == i) {
